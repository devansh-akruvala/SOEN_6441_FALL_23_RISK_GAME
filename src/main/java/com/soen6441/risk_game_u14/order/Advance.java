...
/**
 * The Advance class represents a type of order issued by a player in the game.
 * This order allows a player to attack a territory belonging to another player
 * using a specified number of armies. The attack outcome is determined by a 60%
 * success rate for the attacker and a 70% success rate for the defender. The
 * order format is: "advance sourceCountry targetCountry NumberOfArmies".
 *
...
public class Advance implements Order {
	...
	/**
	 * The country from which the attack is launched (attacker's country) and the
	 * targeted defending country.
	 */
	...
	/**
	 * This method, an implementation of the execute method from the Order
	 * interface, handles the attack mechanism in the game logic. The specific
	 * actions depend on the integer value returned from the isValid method. The
	 * different scenarios include:
	 * <ol>
	 * <li>1. If both the sourceCountry and targetCountry belong to the same player,
	 * the armies simply move from the source to the target country.</li>
	 * <li>2. If the target country has no defending armies, it's acquired by the
	 * attacking player, who deploys the specified attacking armies there.</li>
	 * <li>3. Otherwise, a standard attack takes place.</li>
	 * </ol>
	 * During the attack phase: - Each attacking army is randomly assigned a value
	 * between 0 and 6 (representing a maximum 60% chance of winning the attack). -
	 * Each defending army is randomly assigned a value between 0 and 7
	 * (representing a maximum 70% chance of surviving the attack).
	 * <p>
	 * The battle unfolds based on the relative strengths of attacking and defending
	 * armies: - If the attacking armies outnumber the defending armies, each side
	 * matches their best armies one-on-one. - The side with the higher assigned
	 * value in a one-on-one battle wins. The victorious armies determine the
	 * overall outcome.
	 * <p>
	 * The aftermath is decided based on the number of winning armies: - If the
	 * attacking side has more winning armies, they conquer the target country and
	 * deploy remaining armies, including those uninvolved in the battle. - If the
	 * defender has an equal or greater number of winning armies, they succeed,
	 * maintaining remaining defending armies and those unengaged in the fight.
	 * <p>
	 * In the event of a defender's victory, the remaining attacking armies return
...
//			d_Player.setD_Result("min armies"+l_MinArmies);
	...
	/**
	 * This method determines the armies engaged in the battle. If the initial
	 * number of attacking armies exceeds the defending forces, it selects the top
	 * 'n' armies with the highest assigned values from the original set of armies,
	 * where 'n' is the number of defending armies. Similarly, if the original
	 * defending armies outnumber the attacking forces, it chooses the top 'n'
	 * armies with the greatest corresponding values from the initial set of armies,
	 * with 'n' representing the number of attacking armies. When the quantities of
	 * attacking and defending armies are equal, the method solely returns the
	 * attacking armies to maintain a size reference in the execute method.
	 *
	 * @param p_SizeDiffint    Indicates which side has a numerical advantage in
	 *                         armies.
	 * @param p_MinArmies      Signifies the quantity of armies participating in the
	 *                         battle.
	 * @param p_AttackerArmies Represents the initial attacking armies with randomly
	 *                         assigned values.
	 * @param p_DefenderArmies Denotes the original defending armies with randomly
	 *                         assigned values.
	 * @return The selection of armies based on their maximum corresponding values,
	 *         dependent on which side holds a larger number of armies.
	 */
	...
	/**
	 * This method validates the order and identifies several conditions that render
	 * the order invalid:
	 * <ol>
	 * <li>When the issuing player is currently in negotiations with the owner of
	 * the target country.</li>
	 * <li>When the sourceCountry and the targetCountry are identical.</li>
	 * <li>When the sourceCountry and the targetCountry are not neighboring
	 * territories.</li>
	 * <li>When the sourceCountry is left with fewer than one army after the order
	 * has been executed with a specific number of armies.</li>
	 * </ol>
	 * This method returns different integers based on various valid or invalid
	 * execution cases:
	 * <ol>
	 * <li>It returns an integer of 1 when both countries involved in the order
	 * belong to the issuing player.</li>
	 * <li>An integer of 2 is returned when the countries belong to different
	 * players.</li>
	 * </ol>
	 *
	 * @return An integer representing the different scenarios determining the
	 *         validity of the order's execution.
	 */
	...
	public int isValid() {
		...
		if (d_Player.getD_NegotiatedPlayers().contains(d_TargetCountry.getD_Owner())) {
			d_Player.setD_Result("\nThe targeted country " + d_TargetCountry.getD_CountryName() + " belongs to "
					+ d_TargetCountry.getD_Owner().getD_PlayerName() + " which is negotiated player!");
			l_ReturnInt = 0;
			...
			if (d_Player.getD_PlayerOwnedCountries().contains(d_SourceCountry)
					&& d_Player.getD_PlayerOwnedCountries().contains(d_TargetCountry)) {

				d_Player.setD_Result("\nThe source country and target country belong to the same player");
				l_ReturnInt = 1;
			} else if (d_Player.getD_PlayerOwnedCountries().contains(d_SourceCountry)
					&& !d_Player.getD_PlayerOwnedCountries().contains(d_TargetCountry)) {
				l_ReturnInt = 2;
			}
