...
import com.soen6441.risk_game_u14.state.ExecuteOrder;
import com.soen6441.risk_game_u14.state.IssueOrder;
import com.soen6441.risk_game_u14.state.Phase;
import com.soen6441.risk_game_u14.state.Reinforcement;
...
//				d_GameModel.getD_Map().loadFile(l_MapList[i]);
				...
				try {
					File l_File = new File(l_Path+ l_MapList[i]);
					...
					l_Sc.close();
					if (l_Flag) {
						Target l_TargetObject = new Adapter(new Adaptee(), this);
						l_TargetObject.loadMap(l_MapList[i]);
					} else {
						d_GameModel.getD_Map().loadFile(l_MapList[i]);
					}
				} catch (Exception e) {
...
//				}
				...
				d_GameModel.addPlayers("Player_" + (NUM++) + l_PlayerStrategyList[firstStrategyIndex],
						l_PlayerStrategyList[firstStrategyIndex]);
				d_GameModel.addPlayers("Player_" + (NUM++) + l_PlayerStrategyList[secondStrategyIndex],
						l_PlayerStrategyList[secondStrategyIndex]);
...
//					}
					if (d_PlayerController.checkTheWinner() == 0) {
						System.out.println("Gameover Winner is:" + d_PlayerController.getWinner());
	...
	public void loadGame(String p_Command) {
		...
		System.out.println("\nGame Loaded");
		setD_PlayerController(new PlayerController(d_GameModel));
		setD_MapController(new MapController(d_GameModel.getD_Map()));
	 ...
	 * gameplay. Once the game is saved, this method also sets it phase to GameSaved
	 * phase.
	 * 
	 * @param p_Command command entered by the user
	 */
	public void saveGame(String p_Command) {
//		this.d_GameModel.saveGame1(p_Command.split(" ")[1]);
		this.d_GameModel.saveGame(p_Command.split(" ")[1]);
		this.setD_GamePhase(new SaveGame(this));
	}
