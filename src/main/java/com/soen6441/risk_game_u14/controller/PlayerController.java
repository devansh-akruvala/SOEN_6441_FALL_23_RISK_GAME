...
public class PlayerController implements Serializable{
		...
		if (l_CommandSplit[0].equalsIgnoreCase("gameplayer")) {
			...
			while (l_WordIndex < l_CommandSplitLength) {
				if (l_CommandSplit[l_WordIndex].equalsIgnoreCase("-add") && l_WordIndex + 2 < l_CommandSplitLength) {
					try {
						d_GameModel.addPlayers(l_CommandSplit[l_WordIndex + 1], l_CommandSplit[l_WordIndex + 2]);
					} catch (Exception e) {
						return e.getMessage();
					}
					l_WordIndex += 3;
	...
	/**
	 * Manages the issuance of orders by players during the game. This method
	 * handles the order issuing process for players in the game.
	 */
...
//    }
	public void playerIssueOrder() {
//		ArrayList<Player> l_Players = d_GameModel.getD_Players();
...
//
//		for (Player l_TempPlayer : l_Players) {
//			l_TempPlayer.setIsExit(false);
//			boolean condition = !"Neutral Player".equals(l_TempPlayer.getD_PlayerName())
//					&& "com.soen6441.risk_game_u14.strategy.HumanPlayerStrategy"
//							.equals(l_TempPlayer.getD_PlayerStrategy().toString().split("@")[0]);
//			if (condition) {
//				l_HumanCounter++;
//			}
//		}
...
//
//				if (!"Neutral Player".equals(l_Player.getD_PlayerName())) {
//
//				
////					System.out.println("in player controller - "+l_Player.getD_PlayerStrategy().toString().split("@")[0]);
//					if ("org.soen6441.strategypattern.HumanPlayerStrategy"
//							.equals(l_Player.getD_PlayerStrategy().toString().split("@")[0])) {
//						((HumanPlayerStrategy) l_Player.getD_PlayerStrategy()).setCheckArmies(l_CheckArmies);
//						l_CheckArmies = (HashMap<Player, Boolean>) ((HumanPlayerStrategy) l_Player
//								.getD_PlayerStrategy()).getCheckArmies();
//
//					}
////					else if("org.soen6441.strategypattern.RandomPlayerStrategy".equals(l_Player.getD_PlayerStrategy().toString().split("@")[0]))
////					{
////						System.out.println("in player controller before issue order checkarmies-"+l_CheckArmies);
////						((RandomPlayerStrategy)l_Player.getD_PlayerStrategy()).setCheckArmies(l_CheckArmies);
////						l_CheckArmies = (HashMap<Player,Boolean>)((RandomPlayerStrategy)l_Player.getD_PlayerStrategy()).getCheckArmies();	
////					}
//
//					if (l_Player.getIsExit() == false) {
//
...
//						if ("org.soen6441.strategypattern.HumanPlayerStrategy"
//								.equals(l_Player.getD_PlayerStrategy().toString().split("@")[0])) {
//							l_decreasePlayerListSize = ((HumanPlayerStrategy) l_Player.getD_PlayerStrategy())
//									.getDecreasePlayerListSize();
//							l_CheckArmies = (HashMap<Player, Boolean>) ((HumanPlayerStrategy) l_Player
//									.getD_PlayerStrategy()).getCheckArmies();
...
////							else if("org.soen6441.strategypattern.RandomPlayerStrategy".equals(l_Player.getD_PlayerStrategy().toString().split("@")[0]))
////							{
////								l_decreasePlayerListSize= ((RandomPlayerStrategy)l_Player.getD_PlayerStrategy()).getDecreasePlayerListSize();
////								l_CheckArmies = (HashMap<Player,Boolean>)((RandomPlayerStrategy)l_Player.getD_PlayerStrategy()).getCheckArmies();
////								System.out.println("in player controller after issue order checkarmies-"+l_CheckArmies);
////							}							
//					}
//					if (l_Player.getIsExit() == true) {
//						l_PlayerListSize--;
//					}
//
//				}
...
//		}
	...
	public void removePlayerWithNoCountry() {
		...
		while (l_PlayerIterator.hasNext()) {
			...
			if (l_TempPlayer.getD_PlayerOwnedCountries().size() <= 0
					&& !l_TempPlayer.getD_PlayerName().equals("Neutral Player")) {
